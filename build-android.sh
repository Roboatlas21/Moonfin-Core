...
python3 - "$MEDIA3_DIR/libraries/exoplayer_hls/src/main/java/androidx/media3/exoplayer/hls/HlsMediaChunk.java" <<'PYMEDIA3'
...
if "MoonfinHlsTrace" in s:
    raise SystemExit("HlsMediaChunk instrumentation already present")
...
end = s.index("\n  private void maybeLoadInitData()", start)
...
new = """  @Override
  public void load() throws IOException {
    ...
    boolean traceReusedExtractor =
        extractor == null && previousExtractor != null && previousExtractor.isReusable();
...
p.write_text(s)
PYMEDIA3
echo "MEDIA3 TRACE: HlsMediaChunk.load() instrumentation installed"
